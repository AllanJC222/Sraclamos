...
class ReclamoController extends Controller
{
    ...
    {
        $request->validate([
            'pdf_final' => 'required|mimes:pdf|max:5120', // Máx. 5MB
        ]);
    ...
    public function downloadPdfFinal($id)
    {
        ...
        // 1. Comprobar que el reclamo esté 'Resuelto' y tenga una ruta de PDF final
        if ($reclamo->EstadoReclamo !== 'Resuelto' || empty($reclamo->RutaFirma)) {
            return back()->with('error', 'El reclamo aún no ha sido resuelto o no tiene un PDF final asociado.');
        }
        ...
        if (!Storage::exists($rutaCompleta)) {
            return back()->with('error', 'El archivo PDF final no se encontró en el servidor.');
        }
